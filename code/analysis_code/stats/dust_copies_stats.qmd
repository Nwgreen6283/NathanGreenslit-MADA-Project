...
master %>% 
  ggplot() + geom_histogram(
    aes(
      x = log(copies_mL))) +
  labs(title = "LOG-TRANS")+ 
  facet_wrap(~site)
```

**We will use the log-transformed data as it has a \~ normal distribution.**
...
#master <- 
 # master %>%
 # mutate(copies_mL = log(master$copies_mL))
```
...
```{r}
master2<- dust0 %>% full_join(data) %>%
  filter(hr_cst %in% "01") %>%
  select(site, sal, temp, amm, nn,orthop, copies_mL, t7sum) %>%
  na.omit()
```
