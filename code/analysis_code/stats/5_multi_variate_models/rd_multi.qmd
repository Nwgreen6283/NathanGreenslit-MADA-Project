...
```{r}
fold <- vfold_cv(b, v = 5, repeats = 5) #Data was too small to stratify (<20)
```
...
null_fit<-
  fit_resamples(null_wflow, resamples = fold)
...
aic0<-AIC(null_mod) %>% round(digits = 2)
```
...
kable(data.frame(Model = c("Null Model", "Model 1: Dust", "Model 2: Water Chemistry", "Model 3: Chemistry + Dust", "Model 4: Model 3 + Nutrients", "Model 5: Model 4 + DOM"),
      ...
      caption = "Gulf: Comparative linear regression analysis of Vibrio copies per mL between 6 operational models.")
```
