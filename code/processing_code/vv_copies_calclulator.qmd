...
```{r}
site<- sq %>%
  mutate(site = substr(sq$Sample,1,2)) #Creates new column `Site` from the first two letters in `Sample` (BO16 becomes BO)
...
```{r}
dilution<- site%>%
  mutate(DIL = case_when(site == "BO"~"10",
                              site  == "C2"~ "1",
                              site == "RD"~"1"), 
...
filter<- dilution %>%
   mutate(FIL = case_when(Sample == "BO_7"~"100",
                          Sample == "BO_8"~"100",
                          Sample == "BO_9"~"100",
                          Sample == "BO_9"~"100",
                          Sample == "BO_11"~"100",
